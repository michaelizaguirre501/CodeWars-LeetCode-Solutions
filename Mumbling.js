...
// the second block i will iterate thru my array and push each element into array 2 repeated the times based on its indexed position
...
//i will do that by iterating thru arr2 and pushing [0] to uppercase + the rest of that string
// finally i will return the joined array with - inbetween becasue that is the formatting required
...
function accum(s) {
    ...
    arr.forEach((x, i) => {
        arr2.push(x.repeat(i + 1))
    })

    arr2.forEach(x => {
        finalArr.push(x[0].toUpperCase() + x.substring(1))
    })
